// Minimap Mod
// Shows a minimap in the corner with player positions

const MINIMAP_SIZE = 150;
const MINIMAP_PADDING = 10;

registerHook('onRender', (ctx, camera, dt) => {
  const state = game.getState();
  const canvas = game.getCanvas();
  const myPlayer = state.players.find(p => p.id === game.getPlayerId());

  if (!myPlayer) return;

  // Minimap position (bottom right)
  const mapX = canvas.width - MINIMAP_SIZE - MINIMAP_PADDING;
  const mapY = canvas.height - MINIMAP_SIZE - MINIMAP_PADDING;

  // Get world bounds from config (assuming 2000x2000)
  const worldWidth = 2000;
  const worldHeight = 2000;

  // Draw minimap background
  ctx.fillStyle = 'rgba(10, 10, 20, 0.8)';
  ctx.fillRect(mapX, mapY, MINIMAP_SIZE, MINIMAP_SIZE);

  // Draw border
  ctx.strokeStyle = '#66ccff';
  ctx.lineWidth = 2;
  ctx.strokeRect(mapX, mapY, MINIMAP_SIZE, MINIMAP_SIZE);

  // Draw grid
  ctx.strokeStyle = 'rgba(102, 204, 255, 0.2)';
  ctx.lineWidth = 1;
  for (let i = 1; i < 4; i++) {
    const gridX = mapX + (MINIMAP_SIZE / 4) * i;
    const gridY = mapY + (MINIMAP_SIZE / 4) * i;
    ctx.beginPath();
    ctx.moveTo(gridX, mapY);
    ctx.lineTo(gridX, mapY + MINIMAP_SIZE);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(mapX, gridY);
    ctx.lineTo(mapX + MINIMAP_SIZE, gridY);
    ctx.stroke();
  }

  // Draw pickups
  state.pickups.forEach(pickup => {
    if (!pickup.active) return;

    const px = mapX + (pickup.x / worldWidth) * MINIMAP_SIZE;
    const py = mapY + (pickup.y / worldHeight) * MINIMAP_SIZE;

    const colors = {
      health_small: '#ff9999',
      health_big: '#ff3366',
      armor_light: '#99ddff',
      armor_heavy: '#66ccff',
      ammo: '#ffaa00'
    };

    ctx.fillStyle = colors[pickup.type] || '#ffffff';
    ctx.beginPath();
    ctx.arc(px, py, 2, 0, Math.PI * 2);
    ctx.fill();
  });

  // Draw players
  state.players.forEach(p => {
    if (p.health <= 0) return;

    const px = mapX + (p.x / worldWidth) * MINIMAP_SIZE;
    const py = mapY + (p.y / worldHeight) * MINIMAP_SIZE;

    const isMe = p.id === game.getPlayerId();

    // Player dot
    ctx.fillStyle = isMe ? '#66ccff' : '#ff9966';
    ctx.beginPath();
    ctx.arc(px, py, isMe ? 4 : 3, 0, Math.PI * 2);
    ctx.fill();

    // Direction indicator for local player
    if (isMe) {
      ctx.strokeStyle = '#66ccff';
      ctx.lineWidth = 2;
      const dirLength = 8;
      const endX = px + Math.cos(p.aimAngle) * dirLength;
      const endY = py + Math.sin(p.aimAngle) * dirLength;
      ctx.beginPath();
      ctx.moveTo(px, py);
      ctx.lineTo(endX, endY);
      ctx.stroke();
    }
  });

  // Minimap label
  ctx.fillStyle = '#66ccff';
  ctx.font = 'bold 10px monospace';
  ctx.textAlign = 'left';
  ctx.fillText('MAP', mapX + 5, mapY - 5);
});

console.log('✅ Minimap loaded');
